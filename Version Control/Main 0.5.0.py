...
and integrates various points of interest such as banks, taverns, transits, and user buildings.
It connects to a MySQL database to retrieve the data and uses PyQt5 for the GUI.
"""
...
def connect_to_database():
    """
    Connects to the local MySQL database. If the connection fails, it tries to connect to the remote MySQL database.

    Returns:
        connection (pymysql.connections.Connection): The MySQL database connection object.
    """
    try:
...
def load_data():
    ...
    Returns:
        tuple: Columns and rows mappings, coordinates of banks, taverns, transits, user buildings, and color map.
    """
    ...
    color_map = {type_: color for _, type_, color in color_mappings_data}
...
class CityMapApp(QMainWindow):
    ...
    def on_webview_load_finished(self):
        """
        Slot for handling the load finished signal of the web view. Extracts coordinates from the loaded HTML.
        """
        self.website_frame.page().toHtml(self.process_html)

    def process_html(self, html):
        """
        Processes the HTML content of the web page and extracts coordinates.
    ...
    def extract_coordinates_from_html(self, html):
        ...
        Returns:
            tuple: Coordinates (x, y) extracted from the HTML.
        """
    ...
    def refresh_webview(self):
        """
        Refreshes the web view by reloading the web page.
        """
        self.website_frame.reload()

    def draw_minimap(self):
        """
        Draws the minimap with banks, taverns, transits, and user buildings.
        """
        ...
        def draw_location(column_index, row_index, color, label_text=None):
            ...
            if label_text and self.zoom_level < 7:
            ...
            for j in range(self.zoom_level):
                ...
                if column_index < min(columns.values()) or column_index > max(columns.values()) or row_index < min(
                    ...
                    painter.fillRect(x0 + border_size, y0 + border_size, block_size - 2 * border_size,
                                     block_size - 2 * border_size, QColor('grey'))
                elif (column_index % 2 == 1) or (row_index % 2 == 1):
                    painter.fillRect(x0 + border_size, y0 + border_size, block_size - 2 * border_size,
                                     block_size - 2 * border_size, QColor('darkgrey'))
                else:
                    painter.fillRect(x0 + border_size, y0 + border_size, block_size - 2 * border_size,
                                     block_size - 2 * border_size, QColor('lightgrey'))

                if column_name and row_name and self.zoom_level < 7:
        ...
        for (column_index, row_index) in banks_coordinates:
            draw_location(column_index + 1, row_index + 1, QColor(color_map["bank"]), "OmniBank")

        for name, (column_index, row_index) in taverns_coordinates.items():
            draw_location(column_index + 1, row_index + 1, QColor(color_map["tavern"]), name)

        for name, (column_index, row_index) in transits_coordinates.items():
            draw_location(column_index + 1, row_index + 1, QColor(color_map["transit"]), name)

        for name, (column_index, row_index) in user_buildings_coordinates.items():
            draw_location(column_index + 1, row_index + 1, QColor(color_map["user_building"]), name)
    ...
    def find_nearest_location(self, x, y, locations):
        ...
        Args:
            ...
            locations (list): A list of location coordinates.

        Returns:
            list: Sorted list of distances and corresponding locations.
        """
        distances = []
    ...
    def find_nearest_tavern(self, x, y):
        """
        Finds the nearest tavern to the given coordinates.
        ...
        Returns:
            list: Sorted list of distances and corresponding tavern locations.
        """
        return self.find_nearest_location(x, y, list(taverns_coordinates.values()))

    def find_nearest_bank(self, x, y):
        """
        Finds the nearest bank to the given coordinates.
        ...
        Returns:
            list: Sorted list of distances and corresponding bank locations.
        """
        return self.find_nearest_location(x, y, banks_coordinates)

    def find_nearest_transit(self, x, y):
        """
        Finds the nearest transit to the given coordinates.
        ...
        Returns:
            list: Sorted list of distances and corresponding transit locations.
        """
        return self.find_nearest_location(x, y, list(transits_coordinates.values()))

    def set_destination(self):
        """
        Sets the destination to the current coordinates.
        """
        current_x, current_y = self.column_start + self.zoom_level // 2, self.row_start + self.zoom_level // 2
    ...
    def save_destination(self):
        """
        Saves the current destination to a pickle file.
        """
    ...
    def load_destination(self):
        """
        Loads the destination from a pickle file.
        """
        try:
    ...
    def go_to_location(self):
        """
        Goes to the selected location based on the combo box values.
        """
