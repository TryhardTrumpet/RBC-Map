...
class CityMapApp(QMainWindow):
    ...
    def extract_coordinates_from_html(self, html):
        ...
        # Extract the coordinates from hidden input fields
        move_buttons = soup.find_all('input', {'name': 'action', 'value': 'move'})
        for button in move_buttons:
            x_input = button.find_next('input', {'name': 'x'})
            y_input = button.find_next('input', {'name': 'y'})
            if x_input and y_input:
                x_coord = int(x_input['value'])
                y_coord = int(y_input['value'])
